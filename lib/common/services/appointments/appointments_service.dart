...
class AppointmentsService {
  ...
  static List<Appointment> getAppointmentsByDateRange(DateTime start, DateTime end) {
    if (_box == null || !_box!.isOpen) return [];
    return _box!.values
        .where((appointment) => 
            appointment.date.isAfter(start.subtract(const Duration(days: 1))) &&
  ...
  static Map<String, int> getAppointmentStats() {
    ...
    return {
      ...
      'scheduled': appointments.where((a) => a.status == AppointmentStatus.scheduled).length,
      ...
      'today': appointments.where((a) => 
          ...
          a.date.day == today.day).length,
      'thisWeek': appointments.where((a) => 
          a.date.isAfter(today.subtract(Duration(days: today.weekday - 1))) &&
          a.date.isBefore(today.add(Duration(days: 7 - today.weekday + 1)))).length,
    };
  }

  // Verificar conflitos de horário
  static bool hasTimeConflict(DateTime date, String time, int duration, {String? excludeId}) {
    ...
    for (final appointment in appointments) {
      ...
      final existingEndTime = existingStartTime.add(Duration(minutes: appointment.duration));
